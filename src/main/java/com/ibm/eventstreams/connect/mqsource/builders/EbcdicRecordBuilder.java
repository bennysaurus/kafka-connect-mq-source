 ...
 * </ul>
 */
public class EbcdicRecordBuilder extends BaseRecordBuilder {
    private static final Logger log = LoggerFactory.getLogger(EbcdicRecordBuilder.class);

    public EbcdicRecordBuilder() {
        log.info("Building records using com.ibm.eventstreams.connect.mqsource.builders.EbcdicRecordBuilder");
    }
    ...
    @Override public SchemaAndValue getValue(JMSContext context, String topic, boolean messageBodyJms, Message message) throws JMSException {
        ...
        try {
            if (messageBodyJms) {
                ...
                log.info("Bytes message with OPTIONAL_BYTES schema");
